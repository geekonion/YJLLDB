...
def trace_all_functions_in_module(debugger, command, result, internal_dict):
    ...
    implemented in YJLLDB/src/TraceModule.py
    """
    # 去掉转义符
    ...
    if not lookup_module_name_or_addr and not options.system:
    ...
    for module in target.module_iter():
        ...
        mod_dir = module_file_spec.GetDirectory()

        if options.system:
            if '/usr/lib/system' not in mod_dir:
                continue
        else:
            if is_addr:
                header_addr = 0
    ...
    if module_found:
        if options.individual:
            print("begin trace with {} breakpoint".format(total_count))
    else:
        print("module {} not found".format(lookup_module_name_or_addr))

    if module_count > 1:
        if lookup_module_name_or_addr:
            print("{} modules {} found".format(module_count, lookup_module_name_or_addr))
        else:
            print("{} modules found".format(module_count))
...
def generate_option_parser():
    ...
    parser.add_option("-s", "--system",
                      ...
                      default=False,
                      dest="system",
                      help="trace libs under /usr/lib/system")
